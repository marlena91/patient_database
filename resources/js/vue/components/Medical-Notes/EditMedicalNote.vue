<template>
  <div>
    <h5 class="">Aktualizuj dokumentację medyczną:</h5>
...
export default {
  ...
  methods: {
    async showMedicalNote() {
      await axios
        .get(`/api/medical-notes/${this.$route.params.id}`)
        .then((response) => {
    ...
    async update() {
      await axios
        .put(`/api/medical-notes/${this.$route.params.id}`, this.medicalNote)
        .then((response) => {
          this.$router.push({
            name: "patient",
            params: { id: this.medicalNote.patients_id },
          });
        })
        .catch((error) => {
          console.log(error);
